...
from collections import deque  # For efficient averaging
...
def main(video_path=None, offset_threshold=100, neck_angle_threshold=25, torso_angle_threshold=10, time_threshold=180):
    ...
    while True:
        ...
        # ====== NEW: Performance monitoring calculations ======
        ...
        # Update frame times and calculate average FPS
        frame_times.append(current_fps)
        avg_fps = sum(frame_times) / len(frame_times) if frame_times else current_fps
        ...
        # Display performance metrics (top-right corner)
        perf_x = w - 250
        cv2.putText(image, f"FPS: {current_fps:.1f} | Avg: {avg_fps:.1f}",
                   (perf_x, 30), font, 0.6, yellow, 2)
        cv2.putText(image, f"CPU: {cpu_usage:.1f}% | RAM: {ram_usage:.1f}%",
                   (perf_x, 60), font, 0.6, yellow, 2)
        # ======================================================

        # Use lm and lmPose as representative of the following methods.
        lm = keypoints.pose_landmarks
        if not lm:
            continue  # Skip frame if no landmarks detected
        ...
        if offset < offset_threshold:
            cv2.putText(image, str(int(offset)) + ' Shoulders aligned', (w - 280, 90), font, 0.6, green, 2)
        else:
            cv2.putText(image, str(int(offset)) + ' Shoulders not aligned', (w - 280, 90), font, 0.6, red, 2)
